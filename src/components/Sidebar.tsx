  ...
  const sidebarContent = (
    <nav className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-sidebar-border flex gap-2">
        <Link
          to="/buscar"
          className="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-md bg-sidebar-accent text-sidebar-accent-foreground hover:bg-wheat transition-colors text-sm"
              ...
              {section.recipes && section.recipes.length > 0 ? (
                <div>
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center justify-between py-3 px-4 rounded-lg text-left text-base font-medium transition-colors min-h-[48px]",
                      ...
                      >
                        ...
                        <li>
                          <Link
                            ...
                            onClick={() => setIsOpen(false)}
                            className={cn(
                              "block py-3 px-4 rounded-lg text-base transition-colors min-h-[44px] flex items-center",
                              location.pathname === `/seccion/${section.id}`
                ...
                <Link
                  ...
                  onClick={() => setIsOpen(false)}
                  className={cn(
                    "flex items-center gap-3 py-3 px-4 rounded-lg text-base font-medium transition-colors min-h-[48px]",
                    isActiveSection(section.id)
                      ? "bg-primary text-primary-foreground"
                      : "text-sidebar-foreground hover:bg-sidebar-accent"
                  )}
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 lg:hidden p-3 rounded-lg bg-background shadow-lg border border-border min-w-[48px] min-h-[48px] flex items-center justify-center"
      ...
      {/* Desktop sidebar */}
      <aside className={cn(
        "hidden lg:flex lg:flex-col lg:w-80 lg:fixed lg:inset-y-0 lg:left-0 bg-sidebar border-r border-sidebar-border",
        className
      )}>
        {sidebarContent}
      </aside>
